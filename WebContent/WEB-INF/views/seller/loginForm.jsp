...
<script type="text/javascript">
	$(document).ready(function() {
		...
		inputData("[name=s_id]", '${cookie.s_id.value}');
		inputData("[name=s_pwd]", '${cookie.s_pwd.value}');

		<c:if test="${!empty cookie.s_id.value}">
	...
	function checkLoginForm() {
		...
		$.ajax({
			...
			success : function(data) {
				...
				} else if (data.sellerCnt == "0") {
					alert("아이디, 암호 존재하지 않음. 재입력바람");
				} else if (data.sellerCnt == "-3"){
					alert("해당아이디는 " + data.stopSeller.start_date + "부터 " + data.stopSeller.end_date + "까지" + "정지 되었습니다.\n사유 : " + data.stopSeller.stop_comment );
				}else {
...
<script>
function popupId(){
	var url = "${ctSellerRoot}/forgetSellerId.do";
    var name = "forgetSellerId";
    var option = "width = 500, height = 550, top = 70, left = 200, location = no"
    window.open(url, name, option);
}
function popupPwd(){
	var url = "${ctSellerRoot}/forgetSellerPwd.do";
    var name = "forgetSellerPwd";
		...
		<div class="container-login100">
			<div class="wrap-login100 p-t-50 p-b-90">
				<form class="login100-form validate-form flex-sb flex-w"
					...
					<span class="login100-form-title p-b-51"> JAVABANG - 팔아방 Login </span>

					<%-- <form name="loginForm" method="post" action="${ctBuyerRoot}/homeForm.do"> --%>
					<div class="wrap-input100 validate-input m-b-16"
						data-validate="Username is required">
						<input class="input100" type="text" name="s_id"
					...
					<div class="wrap-input100 validate-input m-b-16"
						...
						<input class="input100" type="password" name="s_pwd"
					...
					<div class="flex-sb-m w-full p-t-3 p-b-24">
						...
						<div>
							...
							<a href="javascript:popupPwd()" class="txt1"> PASSWORD찾기 </a>
						</div>
					</div>
				...
				<div class="container-form-btn m-t-2">
					<button class="btn btn-danger" name="signup"
						onclick="location.replace('${ctSellerRoot}/signupForm.do')"
