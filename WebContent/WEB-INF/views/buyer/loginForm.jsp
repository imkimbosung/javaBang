...
<script type="text/javascript">
	$(document).ready(function() {
		...
		inputData("[name=b_id]", '${cookie.b_id.value}');
		inputData("[name=b_pwd]", '${cookie.b_pwd.value}');

		<c:if test="${!empty cookie.b_id.value}">
		...
		$.ajax({
			...
			success : function(data) {
				...
				if (data.buyerCnt == "1") {
					...
					//oldPage();
					//alert("${uri}");
					if("${buyer_uri}"=="/javaBang/buyer/loginForm.do"){location.replace("${ctBuyerRoot}/homeForm.do");}
					else{location.replace("${buyer_uri}");}
					
				} else if (data.buyerCnt == "0") {
					alert("아이디, 암호 존재하지 않음. 재입력바람");
				} else if(data.buyerCnt == "-3"){
					alert("해당아이디는 " + data.stopBuyer.start_date + "부터 " + data.stopBuyer.end_date + "까지" + "정지 되었습니다.\n사유 : " + data.stopBuyer.stop_comment );
				}
				else {
...
function popupId(){
	...
	var url = "${ctBuyerRoot}/forgetBuyerId.do";
    var name = "forgetBuyerId";
...
function popupPwd(){
	...
	var url = "${ctBuyerRoot}/forgetBuyerPwd.do";
		...
		<div class="container-login100">
			<div class="wrap-login100 p-t-50 p-b-90">
				<form class="login100-form validate-form flex-sb flex-w"
					...
					<span class="login100-form-title p-b-51">JAVABANG - 구해방Login</span>
